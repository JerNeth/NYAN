...
uint32_t nyan::TextureManager::get_texture_idx(const std::string& name, const std::string& defaultTex)
{
	...
	Utility::Logger::warning().format("TextureManager: Couldn't load \"{}\" using \"{}\" instead", name, defaultTex);

	if (const auto& res = m_textureIndex.find(defaultTex); res != m_textureIndex.end())
		return res->second;

	Utility::Logger::warning().format("TextureManager: Couldn't load \"{}\"", defaultTex);
...
uint32_t nyan::TextureManager::get_texture_idx(const std::string& name)
{
	...
	Utility::Logger::warning().format("TextureManager: Couldn't load \"{}\"", name);
