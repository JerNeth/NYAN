...
void nyan::StaticMesh::render(vulkan::CommandBufferHandle& cmd)
{
	...
	if (tangentSpace) {
		cmd->set_vertex_attribute(3, 0, vulkan::get_format<Math::ubvec3>());
		cmd->set_vertex_attribute(4, 0, vulkan::get_format<Math::ubvec3>());
...
void nyan::SkinnedMesh::render(vulkan::CommandBufferHandle& cmd)
{
	...
	if (tangentSpace) {
		cmd->set_vertex_attribute(4, 0, vulkan::get_format<Math::ubvec3>());
		cmd->set_vertex_attribute(5, 0, vulkan::get_format<Math::ubvec3>());
		cmd->set_vertex_attribute(6, 0, vulkan::get_format<Math::ubvec3>());
	}
