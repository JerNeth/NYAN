...
int main()
{
	...
	*/
	...
	try {
		...
		ubo_data[0].model = Math::mat44(Math::mat33::rotation_matrix(0, 0, (degrees++) / 100));
		...
		ubo_data[1].model = Math::mat44(Math::mat33::rotation_matrix(0, 0, (degrees++) / 100));
		...
		while (!window.should_close())
		{
			...
			if (!window.is_iconified()) {
				...
				cmd->bind_texture(0, 1, *tex->get_view(), device.get_default_sampler(Vulkan::DefaultSampler::TrilinearClamp));
