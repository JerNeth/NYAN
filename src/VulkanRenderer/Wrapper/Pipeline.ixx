...
export namespace nyan::vulkan::wrapper
{
	...
	public:
		...
		struct BlendAttachment {
			...
			uint32_t srcColorBlend : BlendFactorBitWidth;
			uint32_t dstColorBlend : BlendFactorBitWidth;
			uint32_t colorBlendOp : BlendOpBitWidth;
			uint32_t srcAlphaBlend : BlendFactorBitWidth;
		...
		using DynamicStates = nyan::util::data::bitset<magic_enum::enum_count<DynamicState>(), DynamicState>; // Needs to be less than 64 states to 
		...
		struct PipelineState {
			...
			uint32_t sampleMask : SampleMaskBitWidth{ std::numeric_limits<uint32_t>::max() };
		...
		struct RenderingCreateInfo {
			...
			std::array<uint8_t, MaxAttachments> colorAttachmentFormats{ static_cast<uint8_t>(VK_FORMAT_UNDEFINED) }; //Choose to ignore the higher formats
			uint8_t colorAttachmentCount{ 0 };
			uint8_t depthAttachmentFormat{ static_cast<uint8_t>(VK_FORMAT_UNDEFINED) };
			uint8_t stencilAttachmentFormat{ static_cast<uint8_t>(VK_FORMAT_UNDEFINED )};
		};
	...
	public:
		struct VertexInput {
			...
			std::array<uint8_t, MaxVertexInputs> vertexInputFormats{ static_cast<uint8_t>(VK_FORMAT_UNDEFINED) }; //Choose to ignore the higher formats
		};
		struct Parameters {
