...
bool setup(nyan::glfw::Window& window, nyan::glfw::Library& library) {
    ...
    {
        assert::defaultAssert(graphicsQueues[0].wait_idle());
    }
    ...
    if (!surfaceResult) {
        log::error().message("couldn't create surface");
        return false;
    }
    ...
    if (!swapchainResult) {
        log::error().message("couldn't create swapchain");
        return false;
    }
    ...
    if (!renderPassResult) {
        log::error().message("couldn't create Renderpass");
        return false;
    }
    ...
    for (auto& image : swapchainResult->get_images()) {
        ...
        if (!framebufferResult) {
            log::error().message("couldn't create framebuffer");
            return false;
        }
    ...
    if (vertShaderCode.empty()) {
        log::error().message("couldn't read vert shader");
        return false;
    }
    ...
    if (fragShaderCode.empty()) {
        log::error().message("couldn't read frag shader");
        return false;
    }
    ...
    GraphicsPipeline::PipelineState defaultPipelineState{
        .dynamicState{},
    ...
    if (!pipelineResult) {
        log::error().message("couldn't create pipeline");
        return false;
    }
    ...
    bool swapStale = false;
    for (; !window.should_close();) {
        //if (i != 0)

        do {
            library.poll_events();
        } while (window.is_iconified());

        if (swapStale) [[unlikely]] {
            swapchainResult = Swapchain::create(*logicalDevice, *surfaceResult, Swapchain::Params{ .format {swapchainFormat }, .colorSpace{swapchainColorSpace }, .oldSwapchain{} });
            if (!swapchainResult)
                continue;
            swapStale = false;
            framebuffers.clear();
            ...
            defaultPipelineState.viewport.width =  static_cast<uint16_t>(swapchainResult->get_width()) ;
            defaultPipelineState.viewport.height =  static_cast<uint16_t>(swapchainResult->get_height()) ;
            pipelineParams.pipelineState = defaultPipelineState;
            pipelineResult = VertexShaderGraphicsPipeline::create(*logicalDevice, *pipelineLayoutResult, pipelineParams, &*pipelineCacheResult);
        ...
        if(auto result = swapchainResult->aquire_image(); !result) {
            log::error().message("couldn't acquire");
            swapStale = true;
            if (result.error().get_result() != Error::Type::Suboptimal) {
                continue;
            }
            //return false;
        }
        ...
        if (auto result = commandPoolResult->reset(); !result) {
            log::error().message("couldn't reset");
            return false;
        }

        if (auto beginResult = commandBufferResult->begin(); !beginResult) {
            log::error().message("couldn't begin");
            return false;
        }
        ...
        if (auto endResult = commandBufferResult->end(); !endResult) {
            log::error().message("couldn't end");
            return false;
        }
        ...
        //}
        if (counter == 5000)
            window.set_resolution({1024, 800});
        if (!(counter++ & 1023)) {
            auto end = std::chrono::steady_clock::now();
            auto ms = std::chrono::duration<double, std::milli>{ end - start }.count();
            log::verbose().message("1024 frames passed").format(" in {}ms => {} fps", ms, std::milli::den * 1024.f/ ms);
            start = end;
        }
...
[[nodiscard]] ExitCode better_main([[maybe_unused]] std::span<const std::string_view> args) {
    auto name = "NYAN: Render Usage Sample";
